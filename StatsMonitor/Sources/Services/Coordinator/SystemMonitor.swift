...
final class SystemMonitor {
    ...
    private func pollNetworkProcesses(processCount: Int) {
        ...
        Task { [weak self] in
            ...
            let result = await Task.detached(priority: .utility) {
                ...
                return (
                    processes: ProcessIdentityCache.shared.resolved(monitor.sampleTopProcesses(processCount: processCount)),
    ...
    private func pollProcessDetails(processCount: Int) {
        ...
        Task { [weak self] in
            guard let self else { return }
            let result = await Task.detached(priority: .utility) {
                // GPU sampler 自帶前值（無前值 pid 不輸出），與名稱／icon 解析一起留在背景。
                let gpu = ProcessIdentityCache.shared.resolved(
                    gpuAppUsageSampler.sampleTopApps(processCount: processCount).map { ProcInfo(gpu: $0) }
                )
